...
const options = {
  ...
  watch: process.env.NODE_ENV === 'production ' ? false : true, // 是否需要监听文件并在发生改变时重新编译它们，默认为 process.env.NODE_ENV !== 'production'
  cache: process.env.NODE_ENV === 'production ' ? false : true, // 启用或禁用缓存，默认为 true
  cacheDir: '.cache', // 存放缓存的目录，默认为 .cache
  minify: process.env.NODE_ENV === 'production ' ? false : true, // 压缩文件，当 process.env.NODE_ENV === 'production' 时，会启用
  ...
  hmrPort: 3010, // hmr socket 运行的端口，默认为随机空闲端口(在 Node.js 中，0 会被解析为随机空闲端口)
  sourceMaps: process.env.NODE_ENV === 'production ' ? false : true, // 启用或禁用 sourcemaps，默认为启用(在精简版本中不支持)
